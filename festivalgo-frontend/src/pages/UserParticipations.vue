<template>
    <div class="participari-container">
      ...
      <div v-else class="festivaluri-boxuri">
        <div
          ...
          :key="p.id"
          :style="{ backgroundImage: `url(${p.festival.imagePath})` }"
        >
          <div class="info-box">
            <div class="text-stanga">
              <h2 class="nume">{{ p.festival.name }}</h2>
              <p class="perioada">{{ p.festival.startDate }} - {{ p.festival.endDate }}</p>
            </div>
  ...
  .btn-anuleaza {
    background-color: rgba(239, 68, 68, 0.9); /* roșu */
    border: none;
    color: white;
    padding: 8px 14px;
    border-radius: 6px;
